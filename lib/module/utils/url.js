...
export function formatQuerystringParameters(parametersMap) {
  return Array.from(parametersMap.entries()).map(([key, value]) => "".concat(encode(key), "=").concat(encode("".concat(value)))).join('&');
}
export function extractQuerystringParameters(querystring) {
  return querystring.replace('?', '').split('&').map(segment => segment.split('=')).reduce((obj, [key, value]) => {
